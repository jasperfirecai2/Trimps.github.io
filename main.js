...
var Fluffy = {
	...
	getExpReward: function(givingExp, count) {
		...
		if (autoBattle.oneTimers.Battlescruff.owned && game.global.universe == 2){
			reward *= (1 + ((autoBattle.getMaxEnemyBeaten()) / 50));
		}
	...
	expBreakdown: function (what) {
		...
		switch(what){
			...
			case "labs":
				...
				return;
			case "battlescruff":
				elem.innerHTML = 'From the Battlescruff Spire Assault reward. Increases Scruffy XP gained by 2% per level cleared, currently granting ' + prettify(1 + ((autoBattle.getMaxEnemyBeaten()) / 50)) + 'x.';
				return;
			case "mutators":
	...
	rewardConfig: {
		...
		SADailies: {
			get description(){
				var cleared = autoBattle.getMaxEnemyBeaten();
			...
			critChance: function(){
				return (0.0025 * (autoBattle.getMaxEnemyBeaten()));
			},
			attackMod: function(){
				return 1 + (0.04 * (autoBattle.getMaxEnemyBeaten()));
			}
		},
		bigSeeds: {
